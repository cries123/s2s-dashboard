...
const PRODUCTION_LANE_IDS: Set<DepartmentColumnId> = new Set(
...
/**
 * Waiting-queue tickets still unassigned at the overnight sweep. Unlike
 * production-lane carryovers, these must NOT be moved to Down in Shop — that
 * would pull them out of the queue advisors are watching. They're flagged
 * overnight-in-place instead, so `isOvernightRo` picks them up for the visual
 * overnight indicator once dispatched. Timing is enforced by the caller —
 * never run this outside the midnight window.
 */
export function shouldFlagUnassignedOvernightCarryover(ro: DispatchRepairOrder): boolean {
  return (
    !ro.isCompleted &&
    (ro.lifecycleStatus ?? 'active') === 'active' &&
    ro.department === 'unassigned'
...
/** Patch for a waiting-queue ticket carried over from a prior day — stays in the queue, just flagged. */
export function buildUnassignedOvernightFlagPatch(): Partial<DispatchRepairOrder> {
  return {
    lifecycleStatus: 'overnight',
  };
}
