      ...
      <AnimatePresence>
        {activePopups.map((message) => (
          <motion.div
            ...
            className="pointer-events-auto rounded-2xl border border-indigo-500/30 bg-white dark:bg-slate-950/95 shadow-2xl shadow-black/50 backdrop-blur-md overflow-hidden"
          >
            <div className="flex items-start gap-3 p-4">
              ...
              >
                ...
                <p className="text-sm text-slate-900 dark:text-white mt-1 leading-snug break-words">{message.body}</p>
              ...
              <button
                ...
                className={cn(
                  'p-1.5 rounded-lg text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-900/5 dark:hover:bg-white/5 transition-colors shrink-0'
                )}
...
export function DealershipChatPanel({
  ...
  return (
    <div className="fixed inset-0 z-[10050] flex items-end sm:items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-3xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-2xl overflow-hidden flex flex-col max-h-[min(720px,calc(100vh-2rem))]">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div>
            <h2 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">Team chat</h2>
            <p className="text-[10px] text-slate-500 mt-0.5">Messages stay within your dealership</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-lg text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-900/5 dark:hover:bg-white/5"
        ...
        <div className="flex min-h-0 flex-1">
          <aside className="w-56 shrink-0 border-r border-slate-200 dark:border-slate-800 flex flex-col bg-slate-50 dark:bg-slate-950/80">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        active
                          ...
                          : 'hover:bg-slate-900/5 dark:hover:bg-white/5 border border-transparent'
                      )}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-xs font-bold text-slate-900 dark:text-white truncate">{thread.otherName}</p>
              ...
              <button
                ...
                className={cn(
                  ...
                  view === 'new'
                    ...
                    : 'bg-slate-100 dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-800'
                )}
              >
                <PenSquare size={12} />
                New message
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            {view === 'new' ? (
              <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800">
                ...
                <select
                  ...
                  className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 text-sm text-slate-900 dark:text-white"
                >
              ...
              <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-800">
                ...
                <p className="text-sm font-bold text-slate-900 dark:text-white mt-0.5">{activeThreadName}</p>
              </div>
            )}
                      ...
                      <div
                        ...
                        className={cn(
                          ...
                          mine
                            ...
                            : 'mr-auto bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700/60'
                        )}
                      >
                        {!mine ? (
            ...
            <form onSubmit={handleSend} className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-2 shrink-0">
              <textarea
                value={draft}
                ...
                className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 resize-none disabled:opacity-50"
              />
              {error ? <p className="text-xs text-rose-400">{error}</p> : null}
              <button
                type="submit"
                disabled={!composeRecipientUid || !draft.trim() || sending}
                className="w-full py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-400 disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 text-white text-[10px] font-black uppercase tracking-wider transition-colors"
              >
