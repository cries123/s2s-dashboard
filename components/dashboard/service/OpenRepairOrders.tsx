...
function matchesSearch(row: OpenRepairOrderRow, query: string): boolean {
  ...
  // A pure-digit query against the RO number should only match on prefix.
  // Plain substring matching lets a short query like "12" match "812345",
  // "123456", and "912340" alike, which is too noisy to be useful.
  const isNumericQuery = /^\d+$/.test(q);
  const roNumber = (row.roNumber || '').toLowerCase();
  const roMatches = isNumericQuery ? roNumber.startsWith(q) : roNumber.includes(q);
  if (roMatches) return true;
...
export default function OpenRepairOrders({
  ...
  if (!isPbsSyncDealership(currentDealershipId)) {
    return (
      <div className="card-base rounded-2xl border border-slate-200 dark:border-white/5 p-8 text-center">
        <p className="text-sm text-slate-600 dark:text-slate-300">Open repair orders are only available for PBS-integrated stores.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-24 md:pb-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          className="btn-primary bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-900 dark:text-white text-xs sm:text-sm py-2.5 px-4 min-h-[44px] w-full sm:w-auto disabled:opacity-50"
        >
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-brand-primary"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="card-base rounded-2xl border border-slate-200 dark:border-white/5 p-10 text-center">
        ...
        <>
            ...
            {filtered.map((row) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="shrink-0 flex flex-col items-end gap-1">
                      ...
                      <span
                        className={cn(
                          ...
                          row.daysOpen >= 5 ? 'text-amber-500 dark:text-amber-400' : 'text-slate-600 dark:text-slate-300'
                        )}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs border-t border-slate-200 dark:border-white/5 pt-3">
                    <div>
                      <p className="crm-label">Vehicle</p>
                      <p className="text-slate-700 dark:text-slate-200 truncate">{row.vehicleLabel || '—'}</p>
                      {row.vinLast8 ? (
                    ...
                    <div>
                      ...
                      <p className="text-slate-700 dark:text-slate-200 truncate">{row.advisor || '—'}</p>
                    </div>
                    <div>
                      <p className="crm-label">Tech</p>
                      <p className="text-slate-700 dark:text-slate-200 truncate">{row.techNumber || '—'}</p>
                    </div>
                    <div>
                      <p className="crm-label">Promise</p>
                      <p className="text-slate-700 dark:text-slate-200 truncate">{row.datePromisedLabel || '—'}</p>
                    </div>
                    {row.concern ? (
                      <div className="col-span-2">
                        <p className="crm-label">Concern</p>
                        <p className="text-slate-700 dark:text-slate-200">{row.concern}</p>
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <div className="hidden md:block card-base rounded-2xl border border-slate-200 dark:border-white/5 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs sm:text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-900/50 text-[10px] uppercase tracking-wider text-slate-500">
                ...
                {filtered.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300 whitespace-nowrap">{row.tag || '—'}</td>
                          ...
                          {row.customerName ? (
                            <>
                              {hasCrmMatch ? (
                                ...
                                <span className="text-slate-900 dark:text-white font-medium truncate max-w-[10rem] sm:max-w-none">
                                  {row.customerName}
                                </span>
                              )}
                              {row.isWaiting ? (
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300 hidden lg:table-cell whitespace-nowrap">
                        {row.advisor}
                      </td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300 hidden lg:table-cell whitespace-nowrap">
                        {row.techNumber || '—'}
