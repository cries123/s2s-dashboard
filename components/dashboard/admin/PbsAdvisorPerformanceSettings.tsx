  ...
  return (
    <div className="space-y-6 pt-4 border-t border-slate-200 dark:border-white/5">
      <div className="space-y-3">
          ...
          {roster.map((row, idx) => (
            <div key={`${row.id}-${idx}`} className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 dark:text-white"
              />
              <button
                type="button"
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 px-4 py-2 bg-slate-200 dark:bg-slate-800 text-[10px] font-black uppercase rounded-xl text-slate-900 dark:text-white"
          >
            <Plus size={12} />
            Add advisor
          </button>
      ...
      <div className="space-y-3 pt-3 border-t border-slate-200 dark:border-white/5">
        ...
        <p className="text-[10px] text-slate-500 max-w-2xl leading-relaxed">
          When PBS puts a login code in the CSR field (e.g. <strong className="text-slate-600 dark:text-slate-300">01</strong> for
          Frank, <strong className="text-slate-600 dark:text-slate-300">LV4278</strong> for Lemmy), map it to the advisor name on your
          roster. After saving, run{' '}
          <strong className="text-slate-600 dark:text-slate-300">Admin → PBS Sync → Pull changes</strong> to rebuild the breakdown.
        </p>
          ...
          {codeRows.map((row, idx) => (
            <div key={`${row.code}-${idx}`} className="flex flex-col sm:flex-row gap-2 items-stretch">
              <input
                ...
                className="w-full sm:w-36 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-mono text-slate-700 dark:text-slate-200 uppercase"
              />
              <input
                type="text"
                value={row.name}
