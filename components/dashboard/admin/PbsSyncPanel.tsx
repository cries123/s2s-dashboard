...
export function PbsSyncPanel(props: PbsSyncPanelProps) {
  if (!isPbsSyncDealership(props.dealershipId)) {
    return (
      <div className="card-base rounded-2xl border border-slate-200 dark:border-white/5 p-6">
        <p className="text-sm text-slate-600 dark:text-slate-300 font-medium">PBS automated sync is not enabled for this store.</p>
        <p className="text-xs text-slate-500 mt-2 leading-relaxed max-w-2xl">
          PartnerHUB sync only runs for <strong className="text-slate-600 dark:text-slate-300">{PBS_SYNC_DEALERSHIP_NAME}</strong>.
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-slate-500 leading-relaxed max-w-2xl">
        ...
        <strong className="text-slate-600 dark:text-slate-300">Pull changes</strong> for updates since the last
        successful sync; use <strong className="text-slate-600 dark:text-slate-300">Full fleet refresh</strong> only
        ...
        <strong className="text-slate-600 dark:text-slate-300">{PBS_SYNC_DEALERSHIP_NAME}</strong> is modified. A scheduled
        job also runs every morning at 6:00 AM Pacific.
      </p>

      <div className="card-base rounded-2xl border border-slate-200 dark:border-white/5 p-5 space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Database size={16} className="text-brand-primary" />
              <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">{dealershipName}</h3>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                canPullFromPbs
                  ? 'border-slate-300 dark:border-white/10 text-slate-700 dark:text-slate-200 hover:bg-slate-900/5 dark:hover:bg-white/5 disabled:opacity-60'
                  : 'border-slate-300 dark:border-slate-800 text-slate-500 cursor-not-allowed'
              )}
            >
        ...
        {!canPullFromPbs && !statusLoading ? (
          <div className="rounded-xl border border-slate-300/50 dark:border-slate-700/50 bg-slate-100 dark:bg-slate-950/40 p-3 text-xs text-slate-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
          <div className="rounded-xl border border-slate-200 dark:border-white/5 bg-slate-100 dark:bg-slate-950/40 p-3">
