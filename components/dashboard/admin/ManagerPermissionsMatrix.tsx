...
export function ManagerPermissionsMatrix() {
  return (
    <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-white/5">
      ...
      <div className="md:hidden space-y-2">
        {ROWS.map((row) => (
          <div key={row.action} className="card-base rounded-xl p-3 space-y-2.5">
            <p className="text-xs font-bold text-slate-900 dark:text-white">{row.action}</p>
            <div className="grid grid-cols-1 gap-2 text-[11px]">
              <div>
                <p className="crm-label">Manager</p>
                <p className="text-slate-600 dark:text-slate-300">{row.manager}</p>
              </div>
              <div>
                <p className="crm-label">System admin</p>
                <p className="text-slate-600 dark:text-slate-300">{row.admin}</p>
              </div>
              <div>
                <p className="crm-label">Staff</p>
                <p className="text-slate-600 dark:text-slate-300">{row.staff}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="hidden md:block overflow-x-auto rounded-xl border border-slate-200 dark:border-white/5">
        <table className="w-full text-left text-[11px]">
          <thead>
            <tr className="border-b border-slate-200 dark:border-white/5 bg-slate-100 dark:bg-slate-950/80">
          ...
          <tbody>
            {ROWS.map((row) => (
              <tr key={row.action} className="border-b border-slate-200 dark:border-white/5 last:border-0">
                <td className="px-3 py-2.5 text-slate-600 dark:text-slate-300 font-medium">{row.action}</td>
