  ...
  return (
    <div className="space-y-4 animate-in fade-in">
            ...
            <button
              ...
              className={cn(
                ...
                filter === id
                  ...
                  : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800 hover:text-slate-900 dark:hover:text-white'
              )}
            >
              {id}
            </button>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="card-base border border-slate-200 dark:border-white/5 p-12 text-center text-slate-500 text-sm">
            No suggestions yet.
          </div>
        ) : (
          filtered.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className="card-base border border-slate-200 dark:border-white/5 p-5 space-y-3 hover:border-slate-300 dark:hover:border-white/10 transition-colors"
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2 border-t border-slate-200 dark:border-white/5">
                  <div className="text-xs text-slate-500">
                    <span className="font-bold text-slate-600 dark:text-slate-300">{s.username}</span>
