...
const CustomerCard: React.FC<CustomerCardProps> = ({ 
  ...
  return (
    <div className={cn("card-base card-interactive p-0 overflow-hidden border-slate-200 dark:border-slate-800/40 group bg-white dark:bg-slate-900/30 backdrop-blur-sm shadow-xl rounded-2xl transition-all duration-300 hover:shadow-2xl", preferences.crmDisplay.density === "compact" && "text-[95%]")}>
      <div className="p-5">
        <div className="flex justify-between items-start gap-3">
          <div className="flex-1">
            ...
            <button 
              ...
              className="text-xl font-black text-slate-900 dark:text-white hover:text-brand-primary transition-colors text-left leading-none tracking-tight uppercase italic"
            >
            ...
            <div className="flex flex-col gap-1.5 mt-3">
              <a href={`tel:${customer.phone}`} className="flex items-center gap-2 text-[10px] font-bold text-slate-400 hover:text-brand-secondary transition-colors group/link">
                <div className="w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-950 flex items-center justify-center group-hover/link:bg-brand-secondary/10 transition-colors border border-slate-200 dark:border-white/5 shadow-inner">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="w-9 h-9 flex items-center justify-center bg-slate-100 dark:bg-slate-950 text-slate-500 hover:text-brand-primary hover:bg-brand-primary/10 rounded-xl transition-all border border-slate-200 dark:border-white/5 shadow-inner hover:scale-105 active:scale-95"
            ...
            <button 
              onClick={() => onViewProfile(customer)}
              className="w-9 h-9 flex items-center justify-center bg-slate-100 dark:bg-slate-950 text-slate-600 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-all border border-slate-200 dark:border-white/5 shadow-inner hover:scale-105 active:scale-95"
              title="Edit Profile"
            >
              <Edit2 size={14} />
            </button>
        ...
        {isAlert && (
          <div className="mt-4 space-y-2.5">
            ...
            {customer.notes && (
              <div className="px-4 py-3 bg-slate-100 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/50 rounded-xl space-y-0.5">
                <p className="text-[8px] font-black text-slate-500 uppercase tracking-[0.15em]">Internal Account Notes</p>
                <p className="text-[10px] font-medium text-slate-600 dark:text-slate-300 italic">"{customer.notes}"</p>
              </div>
            )}
          </div>
        )}
        
        <div className="flex items-center justify-between mt-4 py-3 bg-slate-100 dark:bg-slate-950/30 px-5 -mx-5 border-y border-slate-200 dark:border-white/5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[11px] font-black text-slate-900 dark:text-white uppercase italic tracking-tight leading-tight">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-black text-slate-900 dark:text-white tabular-nums tracking-tighter mt-0.5">
        ...
        <div className="mt-3.5">
          <button 
            ...
            className="w-full flex items-center justify-between p-2.5 bg-slate-100 dark:bg-slate-950/20 hover:bg-slate-200 dark:hover:bg-slate-950/40 rounded-xl border border-slate-200 dark:border-slate-800/30 transition-all text-[10px]"
          >
            <div className="flex items-center gap-2">
              <Wrench size={12} className="text-brand-secondary" />
              <span className="text-[9px] font-black text-slate-900 dark:text-white uppercase tracking-widest">View Maintenance Roadmap</span>
            </div>
          ...
          {showMaintenance && (
            <div className="mt-2 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="p-3 bg-slate-100 dark:bg-slate-950/50 rounded-xl border border-slate-200 dark:border-slate-800/30">
                ...
                <div className="space-y-1.5">
                  {maintenanceTasks.map((task, idx) => (
                    <div key={idx} className="flex items-center justify-between gap-2 p-2 bg-slate-100 dark:bg-slate-900/40 rounded-lg">
                      <div className="flex items-center gap-2">
                        <div className={cn(
                          "w-1 h-1 rounded-full",
                          task.importance === 'high' ? "bg-rose-500" : task.importance === 'medium' ? "bg-amber-500" : "bg-slate-600"
                        )}></div>
                        <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">{task.task}</span>
                      </div>
      ...
      {isAlert && (
        <div className="p-6 pt-0 bg-white dark:bg-slate-900/30 border-t border-slate-200 dark:border-slate-800/50">
          <div className="pt-4">
            <ContactLogQuickForm
