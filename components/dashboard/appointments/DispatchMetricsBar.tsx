...
export function DispatchMetricsBar({
    ...
    return (
      <div className="space-y-2">
        <div className="grid grid-cols-2 gap-2 text-[9px] font-black uppercase tracking-wider">
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/80 px-3 py-2">
            <span className="text-slate-500 block">Queue</span>
            <span className="text-slate-900 dark:text-white tabular-nums text-sm">{metrics.queueCount}</span>
          </div>
          <div
            className={cn(
              ...
              overdueCount > 0
                ...
                : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/80'
            )}
          >
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/60 p-4 space-y-4">
      <div className="flex items-center gap-2">
        <Activity size={14} className="text-indigo-400" />
        <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600 dark:text-slate-300">
          Shop Metrics
        </h2>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {Object.entries(DISPATCH_STATUS_COLORS).map(([code, info]) => (
          <span
            key={code}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/80 text-[9px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400"
          >
            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: info.hex }} />
            {info.label}
            <span className="text-slate-900 dark:text-white tabular-nums">{metrics.statusCounts[code as keyof typeof metrics.statusCounts]}</span>
          </span>
        ))}
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {DISPATCH_PRODUCTION_LANES.filter((lane) => metrics.avgLaneWaitMinutes[lane.id] != null).map(
              (lane) => (
                <span
                  key={lane.id}
                  className="text-[9px] font-bold uppercase tracking-wide px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400"
                >
                  {lane.label.split(' ')[0]}{' '}
    ...
    <span
      className={cn(
        ...
        count > 0
          ...
          : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/80 text-slate-500'
      )}
    >
      <span>{label}</span>
    ...
    <div
      className={cn(
        ...
        highlight
          ...
          : 'border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/70'
      )}
    >
