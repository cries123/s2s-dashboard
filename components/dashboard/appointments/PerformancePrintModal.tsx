...
export const PerformancePrintModal: React.FC<PerformancePrintModalProps> = ({
  ...
  useEffect(() => {
    ...
    const apptRef = query(collection(db, apptPath), where('dealershipId', '==', currentDealershipId || 'hyundai'));
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex flex-col items-center justify-start p-4 md:p-8 z-50 overflow-y-auto no-print">
        ...
        <div id="print-controls" className="w-full max-w-5xl flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 p-4 rounded-3xl shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-400">
              <Printer size={18} />
            </div>
            <div>
              <h2 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">Fixed Ops Performance Report</h2>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-0.5">Preview & Print Team Analytics</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              onClick={onClose}
              className="h-11 px-4 bg-slate-900/5 hover:bg-slate-900/10 dark:bg-white/5 dark:hover:bg-white/10 text-slate-900 dark:text-white border border-slate-900/10 dark:border-white/10 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all cursor-pointer flex items-center justify-center gap-1.5"
            >
              <X size={14} />
              Close
            </button>
        ...
        {isLoading ? (
          <div className="flex-1 w-full max-w-5xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-3xl p-12 flex flex-col items-center justify-center min-h-[400px]">
            <Loader2 className="animate-spin text-emerald-500 mb-3" size={32} />
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">Compiling performance aggregates...</p>
          </div>
        ) : (
          <div 
            id="print-sheet" 
