...
export const WeatherWidget: React.FC<WeatherWidgetProps> = ({
  ...
  const getWeatherIcon = (code: number) => {
    ...
    if (code <= 3) return <Cloud className="text-slate-500 dark:text-slate-300" size={32} />;
  ...
  if (loading) return (
    <div className="flex items-center justify-center p-6 bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800">
      <Loader2 className="animate-spin text-slate-500" size={24} />
    </div>
  );

  return (
    <div className="bg-white dark:bg-gradient-to-br dark:from-slate-900 dark:to-slate-950 p-6 rounded-3xl border border-slate-200 dark:border-slate-800/50 shadow-2xl relative overflow-hidden group">
      {/* Background Glow */}
