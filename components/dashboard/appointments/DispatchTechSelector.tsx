  ...
  const menu = open && menuStyle
    ? createPortal(
        <div
          ref={menuRef}
          style={menuStyle}
          className="rounded-lg border border-indigo-500/30 bg-white dark:bg-slate-950 shadow-2xl shadow-black/60 py-1 animate-in fade-in zoom-in-95 duration-150"
          role="listbox"
    ...
    >
      <button
        ...
        className={cn(
          ...
          compact
            ? 'px-1.5 py-0.5 bg-slate-100 dark:bg-slate-950/40 border-slate-200 dark:border-slate-800/50 hover:border-indigo-500/40'
            : 'px-2 py-1.5 bg-slate-100 dark:bg-slate-950/50 border-slate-200 dark:border-slate-800/60 hover:border-indigo-500/40 hover:bg-indigo-950/20'
        )}
      >
        <span
          className={cn(
        ...
        <span
          className={cn(
            'text-slate-700 dark:text-slate-200 font-medium block truncate flex items-center justify-between gap-0.5',
