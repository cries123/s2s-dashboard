...
import { collection, query, where, onSnapshot, doc, updateDoc, writeBatch, setDoc, deleteDoc, deleteField, serverTimestamp, runTransaction, getDocs } from 'firebase/firestore';
...
import {
  ...
  buildUnassignedOvernightFlagPatch,
  isOvernightRo,
  normalizeDispatchOrder,
  shouldFlagUnassignedOvernightCarryover,
...
function playQueueAlert(ctx: AudioContext) {
  try {
...
export function DispatchBoard({ 
  ...
  // Distinct from prevQueueCountRef being 0 — that's ambiguous between "just
  // mounted" and "queue legitimately emptied out." Only the former should
  // suppress the alert on the first snapshot.
  const hasSeenFirstQueueSnapshotRef = useRef(false);
  const tvExitTimerRef = useRef<number | null>(null);
  // Lazily created on first alert (browsers require a user gesture before an
  // AudioContext can be constructed) and reused for every subsequent alert
  // instead of creating/tearing one down on every trigger.
  const queueAlertAudioCtxRef = useRef<AudioContext | null>(null);

  const getQueueAlertAudioContext = useCallback(() => {
    if (!queueAlertAudioCtxRef.current) {
      try {
        queueAlertAudioCtxRef.current = new AudioContext();
      } catch {
        return null;
      }
    }
    return queueAlertAudioCtxRef.current;
  }, []);

  useEffect(() => {
    return () => {
      queueAlertAudioCtxRef.current?.close();
      queueAlertAudioCtxRef.current = null;
    };
  }, []);
  ...
  const executeMidnightSweep = useCallback(async () => {
    ...
    // Waiting-queue tickets get flagged in place, not moved — see
    // shouldFlagUnassignedOvernightCarryover's doc comment.
    const unassignedCarryoversToFlag = scopedOrders.filter(shouldFlagUnassignedOvernightCarryover);

    carryoverSweepInFlightRef.current = true;
    const path = 'artifacts/hyundai-sales-to-service/public/data/dispatchOrders';

    try {
      if (carryoversToReset.length > 0 || unassignedCarryoversToFlag.length > 0) {
        const batch = writeBatch(db);
        carryoversToReset.forEach((ro) => {
          batch.update(doc(db, path, ro.id), buildOvernightDownInShopPatch());
        });
        unassignedCarryoversToFlag.forEach((ro) => {
          batch.update(doc(db, path, ro.id), buildUnassignedOvernightFlagPatch());
        });
        await batch.commit();
        const parts = [];
        if (carryoversToReset.length > 0) {
          parts.push(`moved ${carryoversToReset.length} ticket(s) from lanes to Down in Shop`);
        }
        if (unassignedCarryoversToFlag.length > 0) {
          parts.push(`flagged ${unassignedCarryoversToFlag.length} waiting-queue ticket(s) as overnight`);
        }
        showNotification?.(`End of day: ${parts.join(' and ')}.`);
      }
  ...
  const runMidnightSweepIfDue = useCallback(async () => {
    ...
    const carryoversToReset = scopedOrders.filter(shouldSweepOvernightCarryover);
    const unassignedCarryoversToFlag = scopedOrders.filter(shouldFlagUnassignedOvernightCarryover);
    const totalPending = carryoversToReset.length + unassignedCarryoversToFlag.length;

    if (midnightSweepConfig.mode === 'confirm') {
      setPendingSweepCount(totalPending);
      setSweepConfirmOpen(totalPending > 0);
      return;
    }
  ...
  const handleMoveRo = async (ro: DispatchRepairOrder, target: DispatchMoveTarget) => {
    ...
    try {
      const path = 'artifacts/hyundai-sales-to-service/public/data/dispatchOrders';
      const roRef = doc(db, path, ro.id);
      const patch = buildDispatchMoveUpdate(ro, target, businessDatePst);
      const cap = laneTarget === 'unassigned' ? 0 : laneCapacity[laneTarget as DispatchProductionLane];
      const capacityEnforced = laneTarget !== 'unassigned' && blockWhenFull && !overnightVehicle && !!cap && cap > 0;

      if (capacityEnforced) {
        // The client-side isLaneAtCapacity check above is a fast pre-check against
        // possibly-stale local state — two dispatchers moving into the same
        // near-full lane at once could both pass it. This SDK's transactions can
        // only read specific documents, not a live query, so we query the lane's
        // current occupants just before the transaction, then re-read each of
        // those exact docs inside it — narrowing (not fully eliminating) the race
        // window to the gap between the query and the transaction commit, and
        // catching the common case where one of those specific tickets moved
        // or completed in between. A fully race-proof version would need a
        // maintained per-lane counter document.
        const laneQuery = query(
          collection(db, path),
          where('dealershipId', '==', currentDealershipId),
          where('department', '==', laneTarget),
          where('isCompleted', '==', false)
        );
        const laneSnap = await getDocs(laneQuery);
        const candidateIds = laneSnap.docs.map((d) => d.id).filter((id) => id !== ro.id);

        await runTransaction(db, async (transaction) => {
          const candidateSnaps = await Promise.all(
            candidateIds.map((id) => transaction.get(doc(db, path, id)))
          );
          const stillOccupying = candidateSnaps.filter(
            (snap) => snap.exists() && snap.data()?.department === laneTarget && !snap.data()?.isCompleted
          ).length;
          if (stillOccupying >= cap) {
            throw new Error('LANE_AT_CAPACITY');
          }
          transaction.update(roRef, patch);
        });
      } else {
        await updateDoc(roRef, patch);
      }
      setMoveMenuRoId(null);
      if (showNotification) {
        const label =
      ...
      if (err instanceof Error && err.message === 'LANE_AT_CAPACITY') {
        showNotification?.(
          `${visibleDepartments.find((d) => d.id === laneTarget)?.label || 'Lane'} filled up before this move went through.`,
          true
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    // Any increase triggers the alert — including the common 0 -> 1 case —
    // except on the very first snapshot after mount, which would otherwise
    // fire immediately for whatever tickets were already waiting on load.
    if (hasSeenFirstQueueSnapshotRef.current && queueCount > prevQueueCountRef.current) {
      setQueuePulse(true);
      if (isDisplayMode) {
        const ctx = getQueueAlertAudioContext();
        if (ctx) playQueueAlert(ctx);
      }
      window.setTimeout(() => setQueuePulse(false), 2500);
    }
    hasSeenFirstQueueSnapshotRef.current = true;
    prevQueueCountRef.current = queueCount;
  }, [ticketsByColumn.unassigned.length, isDisplayMode, getQueueAlertAudioContext]);
  ...
  const renderMoveMenuPortal = () => {
    ...
    return createPortal(
      <div
        ...
        style={style}
        className="rounded-xl border border-indigo-500/30 bg-white dark:bg-slate-950 shadow-2xl shadow-black/10 dark:shadow-black/50 overflow-hidden animate-in fade-in zoom-in-95 duration-150"
        onClick={(e) => e.stopPropagation()}
        onPointerDown={(e) => e.stopPropagation()}
        role="menu"
      >
        <p className="px-3 py-2 text-[8px] font-black uppercase tracking-widest text-slate-500 border-b border-slate-200 dark:border-white/5">
          Route #{ro.roNumber}
        </p>
        <div className="max-h-52 overflow-y-auto py-1">
          {moveTargets.map(({ target, label, icon }) => (
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wide text-slate-700 dark:text-slate-200 hover:bg-indigo-500/15 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer"
            >
  ...
  const renderTechDisplayCard = (ro: DispatchRepairOrder) => {
    ...
    return (
      <div
        ...
        className={cn(
          'bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 select-none space-y-1',
      ...
      >
        ...
        <p className="text-[10px] font-bold text-slate-700 dark:text-slate-200 uppercase truncate leading-tight">
          {lastName}
        </p>
  ...
  const renderDisplayCard = (ro: DispatchRepairOrder) => {
    ...
    return (
      <div
        ...
        style={{ borderLeftColor: statusInfo.hex, borderLeftWidth: '4px' }}
        className={cn(
          'relative bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5 select-none space-y-0.5',
      ...
      >
        <div className="flex items-center justify-between gap-1">
          <span className="text-[11px] font-black text-slate-900 dark:text-white tabular-nums truncate">{ro.roNumber}</span>
          {renderIntakeFlagBadge(ro, true)}
        </div>
        <p className="text-[9px] font-bold text-slate-600 dark:text-slate-300 truncate uppercase">
  ...
  const renderRoCard = (ro: DispatchRepairOrder) => {
    ...
    return (
      <div
        ...
        className={cn(
          'bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-950 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700/80 p-4 rounded-xl space-y-3 shadow-lg hover:shadow-2xl hover:shadow-indigo-950/10 transition-all duration-300 relative group select-none w-full text-slate-900 dark:text-slate-100',
      ...
      >
        <div className="flex justify-between items-start gap-2">
          <button
            ...
            className="group flex-1 min-w-0 space-y-1.5 text-left rounded-lg hover:bg-slate-900/5 dark:hover:bg-slate-900/40 transition-colors cursor-pointer p-1 -m-1"
            title="Click to edit RO details"
          >
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xl font-black text-slate-900 dark:text-white tabular-nums tracking-tight leading-none">
                {ro.roNumber}
              </span>
              <span
            ...
            <div>
              <h3 className="text-sm font-bold tracking-tight text-slate-900 dark:text-white truncate">{customerLabel}</h3>
              {vehicleLabel ? (
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="flex items-center gap-1 text-[8px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 px-1.5 py-0.5 rounded hover:bg-indigo-950/40 hover:text-indigo-300 hover:border-indigo-900/40"
              title="Edit RO"
            >
              <Pencil size={11} /> Edit
            </button>
            ...
            {confirmDeleteId === ro.id ? (
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button
                  ...
                  onMouseDown={(e) => e.stopPropagation()}
                  className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-1 py-0.5 rounded hover:bg-slate-200 dark:hover:bg-slate-800 transition-all cursor-pointer relative z-20"
                >
                  No
                </button>
              </div>
        ...
        {ro.concern ? (
          <div className="rounded-lg border border-sky-500/25 bg-sky-950/25 px-3 py-2">
            ...
            <p className="text-xs font-medium text-slate-700 dark:text-slate-100 leading-relaxed break-words">
              {ro.concern}
            </p>
          </div>
        ) : null}
          ...
          <div className="flex flex-wrap gap-1.5">
            {factChips.map((chip) => (
              <span
                ...
                className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 text-[10px]"
              >
                <span className="text-[8px] font-black uppercase tracking-wider text-slate-500">
                  {chip.label}
                </span>
                <span className="font-mono text-slate-700 dark:text-slate-200 truncate max-w-[120px]">{chip.value}</span>
              </span>
            ))}
          </div>
        ) : null}
        ...
        >
          <div className="flex items-stretch gap-2">
            <div className="flex-1 min-w-0 rounded-lg border border-slate-200 dark:border-slate-800/40 bg-slate-100 dark:bg-slate-950/30 px-1.5 py-1">
            ...
            <div className="shrink-0 flex items-center justify-center px-2 py-1 rounded-lg border border-slate-200 dark:border-slate-800/40 bg-slate-100 dark:bg-slate-950/30 max-w-[5.5rem]">
          ...
          <div className="rounded-lg border border-slate-200 dark:border-slate-800/40 bg-slate-100 dark:bg-slate-950/30 p-2.5 space-y-2">
        ...
        <div
          className="flex items-center justify-between gap-2 pt-2 border-t border-slate-200 dark:border-slate-800/60"
          onClick={(e) => e.stopPropagation()}
          onMouseDown={(e) => e.stopPropagation()}
          onPointerDown={(e) => e.stopPropagation()}
        >
          <div className="relative inline-flex items-center flex-1 min-w-0 max-w-[155px]">
            <select
              ...
              onClick={(e) => e.stopPropagation()}
              onMouseDown={(e) => e.stopPropagation()}
              className="text-[10px] font-black uppercase tracking-wider w-full px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 outline-none cursor-pointer focus:border-indigo-500 transition-all appearance-none text-left"
            ...
            >
              {Object.entries(DISPATCH_STATUS_COLORS).map(([val, info]) => (
                <option
                  key={val}
                  value={val}
                  className="font-bold text-xs bg-white dark:bg-slate-950 text-slate-900 dark:text-white py-1"
                >
                  {info.label}
                </option>
              ))}
          ...
          <button
            ...
            className="flex items-center gap-1 bg-slate-100 dark:bg-slate-950 hover:bg-emerald-950/60 hover:text-emerald-400 border border-slate-200 dark:border-slate-800 hover:border-emerald-900/60 px-2.5 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all duration-300 select-none cursor-pointer"
          >
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-slate-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/5 pb-5">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Departmental Dispatch Board</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap justify-end">
          ...
          />
          {showTodayLoad && (
            <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-[10px] font-black uppercase tracking-wider">
              <Calendar size={13} className="text-indigo-400 shrink-0" />
              <span className="text-slate-400">Today</span>
              <span className="text-slate-900 dark:text-white tabular-nums">{activeTickets.filter((o) => o.dateCreated === businessDatePst).length}</span>
          ...
          <button
            ...
            className="btn-secondary border text-xs gap-1.5 font-bold uppercase tracking-wider py-2 px-4 rounded-xl transition-all bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-indigo-500/40 disabled:opacity-40"
          >
            <Monitor size={13} />
          ...
          <button
            ...
            className="btn-secondary border text-xs gap-1.5 font-bold uppercase tracking-wider py-2 px-4 rounded-xl transition-all bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-violet-500/40 disabled:opacity-40"
          >
            <Users size={13} />
          ...
          <button
            ...
            disabled={loading}
            className="btn-secondary border text-xs gap-1.5 font-bold uppercase tracking-wider py-2 px-4 rounded-xl transition-all bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-amber-500/40 disabled:opacity-40"
          >
            <FileText size={13} />
        ...
        <div className="rounded-2xl border border-sky-500/30 bg-sky-950/15 p-4 sm:p-5 space-y-4 shadow-lg shadow-sky-950/10">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              className="text-[10px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
            >
              Close lookup
            </button>
          </div>
          {lookupRo.isCompleted ? (
            <div className="max-w-xl rounded-xl border border-emerald-500/20 bg-slate-100 dark:bg-slate-950/60 p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  ...
                  <span className="text-slate-900 dark:text-white font-bold">{lookupRo.customerName || displayCustomerLastName(lookupRo)}</span>
                </div>
                <div>
                  <span className="text-[9px] font-black uppercase tracking-wider text-slate-500 block">Tech</span>
                  <span className="text-slate-900 dark:text-white font-mono">{lookupRo.techNumber}</span>
                </div>
              </div>
              <button
                type="button"
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl space-y-4">
          ...
          {completedTickets.length === 0 ? (
            <p className="text-slate-500 text-xs font-mono py-6 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs text-slate-400">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 text-[9.5px] font-black uppercase text-slate-500 tracking-wider">
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {completedTickets.map(ro => {
                    ...
                    return (
                      <tr key={ro.id} className="hover:bg-slate-900/5 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="py-3 px-3 font-bold text-slate-700 dark:text-slate-200 tabular-nums">{ro.roNumber}</td>
                        <td className="py-3 px-3 font-mono font-bold text-slate-600 dark:text-slate-300">{ro.techNumber}</td>
                        <td className="py-3 px-3 font-bold text-slate-600 dark:text-slate-300 uppercase">{displayCustomerLastName(ro)}</td>
                        <td className="py-3 px-3">
                          <span className="bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-400 px-2 py-1 rounded-md text-[10px] font-bold uppercase border border-slate-200 dark:border-slate-800">
                            {deptLabel}
                          </span>
                        </td>
        ...
        <div className="hidden md:block space-y-6 w-full pb-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 w-full items-stretch">
            ...
            <div
              className={cn(
                ...
                'border-slate-200 dark:border-white/[0.08] bg-gradient-to-br from-slate-50 via-white to-slate-50 dark:from-slate-900/90 dark:via-slate-950 dark:to-slate-900/80',
            ...
            >
              ...
              <div className="relative p-5 sm:p-6 flex flex-col flex-1 gap-4">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-[0.2em] truncate">Waiting Queue</h2>
                  ...
                  <div className={cn(
                    ...
                    ticketsByColumn.unassigned.length > 0
                      ...
                      : 'bg-slate-100 dark:bg-slate-950/80 border-slate-200 dark:border-slate-800 text-slate-500'
                  )}>
                ...
                <div className={cn(
                  ...
                  ticketsByColumn.unassigned.length === 0
                    ? 'border border-dashed border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/30'
                    : 'border border-slate-200 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-950/40'
                )}>
                  {ticketsByColumn.unassigned.length === 0 ? (
            ...
            {visibleDepartments.map((dept) => {
              ...
              return (
                <div 
                  key={dept.id} 
                  className={cn(
                    "bg-gradient-to-r from-slate-50 to-white dark:from-slate-900/60 dark:to-slate-900/30 border border-slate-200 dark:border-slate-800 rounded-2xl p-4.5 flex flex-col md:flex-row md:items-center gap-5 w-full transition-all duration-300 shadow-md relative",
                    list.length > 0 ? "border-slate-300 dark:border-slate-800/80 bg-slate-100 dark:bg-slate-900/40" : "border-slate-200 dark:border-slate-900/60",
                ...
                >
                  ...
                  <div className="flex items-center justify-between md:flex-col md:items-start md:justify-center gap-1.5 md:w-48 shrink-0 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800/80 pb-3 md:pb-0 md:pr-4 select-none">
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/85 rounded-lg text-indigo-400">
                        <dept.icon size={13} />
                      </div>
                      <h3 className="text-xs font-black text-slate-900 dark:text-slate-100 uppercase tracking-widest font-sans">
                        {dept.label}
                      </h3>
                    </div>
                    {(() => {
                      ...
                      return (
                        <span className={cn(
                          ...
                          atCap ? 'bg-rose-950/50 text-rose-400 border-rose-900/50' : 'bg-slate-100 dark:bg-slate-950 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800'
                        )}>
                  ...
                  <div className="flex-1 flex gap-4 overflow-x-auto pb-2 min-h-[150px] items-center scrollbar-thin">
                    {list.length === 0 ? (
                      <div className="flex items-center gap-2 text-slate-600 py-6 px-3 border border-dashed border-slate-200 dark:border-slate-800/60 rounded-xl w-full">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-wrap items-center justify-between gap-4 select-none">
            <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-4 text-[10px] font-bold">
              {Object.entries(DISPATCH_STATUS_COLORS).map(([code, info]) => (
                <div key={code} className="flex items-center gap-2 bg-slate-100 dark:bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
      ...
      {sweepConfirmOpen && pendingSweepCount > 0 ? (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/70 p-4">
          <div className="card-base max-w-md w-full rounded-2xl border border-amber-500/30 p-6 space-y-4">
            <h2 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">Confirm end-of-day sweep</h2>
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-slate-200 dark:bg-slate-800 text-[10px] font-black uppercase text-slate-600 dark:text-slate-300"
              >
                Cancel
              </button>
              <button
