    ...
    <div className="md:hidden space-y-4 pb-6">
      <div className="flex items-center gap-2">
        <button
          ...
          onClick={goPrev}
          disabled={tabIndex <= 0}
          className="p-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-400 disabled:opacity-30"
      ...
      {activeTab === 'intake' ? (
        <div className="space-y-4">
          {intakeForm}
          <div className="rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-slate-900/80 p-4 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Inbox size={14} className="text-amber-300" />
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-900 dark:text-white">Waiting Queue</span>
              </div>
              <span className="text-[9px] font-black tabular-nums text-amber-200">{queueTickets.length}</span>
            </div>
            {queueTickets.length === 0 ? (
              <p className="text-center text-[10px] font-bold uppercase tracking-wider text-slate-600 py-8 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
                Queue is clear
              </p>
            ) : (
        ...
        <div className="space-y-3">
          ...
          {activeList.length === 0 ? (
            <p className="text-center text-[10px] font-bold uppercase tracking-wider text-slate-600 py-12 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
