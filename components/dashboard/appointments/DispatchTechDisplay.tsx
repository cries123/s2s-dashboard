...
export function DispatchTechDisplay({
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="relative flex flex-col flex-1 min-h-0 p-2 gap-2">
        <button
          ...
          onClick={onClose}
          className={cn(
            'absolute top-2 right-2 z-10 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white/80 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700 text-[9px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:border-slate-400 dark:hover:border-slate-500 transition-opacity duration-500',
            ...
            >
              {row.map((tech) => {
                ...
                return (
                  <div
                    key={tech.id}
                    className="flex flex-col min-w-0 min-h-0 rounded-xl border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-900/50 overflow-hidden"
                  >
                    <div className="shrink-0 px-2 py-1.5 border-b border-slate-200 dark:border-slate-800/80 bg-slate-100 dark:bg-slate-950/80">
                      <p className="text-[8px] font-black uppercase tracking-wider text-indigo-400 truncate">
                        #{tech.id}
                      </p>
                      <div className="flex items-center justify-between gap-1 mt-0.5">
                        <span className="text-[9px] font-bold text-slate-900 dark:text-white truncate leading-tight">
                          {resolveTechDisplayName(tech.id, [tech])}
                        </span>
                        <span className="text-[8px] font-black tabular-nums px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 shrink-0">
                          {list.length}
                        </span>
                      </div>
                    </div>
              ...
              {row.length < TECHS_PER_ROW
                ? Array.from({ length: TECHS_PER_ROW - row.length }).map((_, i) => (
                    <div
                      key={`pad-${rowIndex}-${i}`}
                      className="rounded-xl border border-dashed border-slate-200 dark:border-slate-900/80 bg-slate-50 dark:bg-slate-950/20"
                    />
                  ))
                : null}
            </div>
        ...
        <div className="shrink-0 flex flex-col max-h-[28vh] min-h-[120px] rounded-xl border border-amber-500/30 bg-slate-50 dark:bg-slate-900/60 overflow-hidden">
