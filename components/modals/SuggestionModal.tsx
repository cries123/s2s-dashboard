...
export function SuggestionModal({
  ...
  return (
    <div className="modal-overlay sm:p-4 p-0 !items-start sm:!items-center overflow-y-auto scroll-smooth">
      <div
        className="w-full sm:max-w-lg card-base border border-slate-200 dark:border-white/10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 sm:rounded-3xl rounded-none min-h-[100dvh] sm:min-h-0"
        role="dialog"
        aria-labelledby="suggestion-modal-title"
      >
        <div className="flex items-start justify-between gap-4 p-5 sm:p-6 border-b border-slate-200 dark:border-white/5">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 id="suggestion-modal-title" className="text-lg font-black text-slate-900 dark:text-white">
                Send a suggestion
              </h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4">
          <div className="rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/50 px-4 py-3 text-xs text-slate-500 dark:text-slate-400">
            <p>
              <span className="font-bold text-slate-600 dark:text-slate-300">{user.username}</span>
              {' · '}
              {dealershipName}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-brand-primary resize-y min-h-[140px]"
            />
            <p className="text-[10px] text-slate-500 dark:text-slate-600 text-right">{message.length}/2000</p>
          </div>
