...
import { collection, doc, updateDoc, Timestamp, onSnapshot, query, orderBy } from 'firebase/firestore';
...
// Only these fields are ever exposed to the user in the "Edit Profile" form below.
// Diffing the saved form state against just this known-editable set (rather than
// spreading the entire local `formData` object into the write) ensures we never
// clobber fields that a background process (e.g. a PBS sync writing `recentVisits`)
// may have updated on the document while the modal was open.
const EDITABLE_FIELDS = [
  'firstName', 'lastName', 'phone', 'email',
  'year', 'make', 'model', 'vin', 'vinLast8', 'mileage', 'soldDate',
  'address', 'city', 'state', 'zip',
  'language', 'enableServiceAlert', 'notes',
] as const;
...
export default function ProfileModal({ customer, currentUser, onClose, onDelete }: ProfileModalProps) {
  ...
  const [contactLogEvents, setContactLogEvents] = useState<TimelineEvent[]>([]);
  const [contactLogLoading, setContactLogLoading] = useState(true);
  ...
  // Subscribe to the customer's contact log subcollection so logged calls/texts/campaign
  // actions show up in the activity timeline alongside service visits.
  useEffect(() => {
    setContactLogLoading(true);
    const contactLogRef = collection(
      db, 'artifacts', 'hyundai-sales-to-service', 'public', 'data', 'customers', customer.id, 'contactLog'
    );
    const contactLogQuery = query(contactLogRef, orderBy('timestamp', 'desc'));

    const unsubscribe = onSnapshot(
      contactLogQuery,
      (snapshot) => {
        const events: TimelineEvent[] = snapshot.docs.map((docSnap) => {
          ...
          return {
            ...
            date,
            title: data.outcome || 'Contact Logged',
  ...
  const serviceEvents: TimelineEvent[] = useMemo(
  ...
  const timelineEvents: TimelineEvent[] = useMemo(
    () => [...serviceEvents, ...contactLogEvents],
    [serviceEvents, contactLogEvents]
  );
  const timelineLoading = contactLogLoading;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      EDITABLE_FIELDS.forEach((field) => {
        const nextValue = (formData as any)[field];
      ...
      if (Object.keys(updates).length > 0) {
        await updateDoc(doc(db, 'artifacts', 'hyundai-sales-to-service', 'public', 'data', 'customers', customer.id), updates);
  ...
  return (
    <div className="modal-overlay sm:p-4 p-0 !items-start sm:!items-center overflow-y-auto scroll-smooth">
      <div className="modal-content !max-w-6xl w-full h-auto min-h-[100dvh] sm:min-h-0 sm:h-[90vh] rounded-none sm:rounded-[24px] !bg-white dark:!bg-[#0b0f1a] border border-slate-200 dark:border-white/5 shadow-2xl relative flex flex-col overflow-visible sm:overflow-hidden animate-zoom-in">
        
        {/* Banner with gradient accent */}
        <div className="absolute top-0 left-0 right-0 h-[120px] bg-gradient-to-r from-brand-primary/10 via-brand-secondary/5 to-transparent border-b border-slate-200 dark:border-white/5 opacity-50 z-0 pointer-events-none" />

        {/* Modal Header Dossier Card */}
        <div className="p-4 sm:p-6 md:p-8 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-950/40 relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6 shrink-0">
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2.5 sm:gap-3 flex-wrap">
                <h3 className="text-lg sm:text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight leading-tight">
                  {displayName}
                </h3>
                <span className={cn(
              ...
              <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[11px] sm:text-xs font-medium text-slate-400">
                ...
                <span className="flex items-center gap-1 font-mono text-[10px] sm:text-[11px] bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 px-1.5 sm:px-2 py-0.5 rounded text-brand-secondary" title={customer.vin ? `Full VIN: ${customer.vin}` : `VIN Last 8: ${customer.vinLast8}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full md:w-auto self-stretch md:self-auto justify-end mt-2 md:mt-0">
            ...
            <button 
              onClick={onClose} 
              className="p-2 sm:p-3 text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-lg sm:rounded-xl transition-all ml-1"
            >
              <X size={18} />
            </button>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 bg-slate-100 dark:bg-slate-950/80 shrink-0 border-b border-slate-200 dark:border-white/5 p-3 sm:p-4 md:px-8 gap-2 sm:gap-3">
          <div className="p-2.5 sm:p-3.5 bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] sm:text-xs font-black text-slate-900 dark:text-white mt-0.5 truncate max-w-[100px] sm:max-w-[150px]">{customer.year || 'N/A'} {customer.make} {customer.model}</p>
            </div>
          </div>
          ...
          <div className="p-2.5 sm:p-3.5 bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] sm:text-xs font-black text-slate-900 dark:text-white mt-0.5 truncate">{customer.mileage ? `${parseInt(customer.mileage).toLocaleString()} mi` : 'Not Logged'}</p>
            </div>
          </div>
          ...
          <div className="p-2.5 sm:p-3.5 bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] sm:text-xs font-black text-slate-900 dark:text-white mt-0.5 truncate">{customer.soldDate ? new Date(customer.soldDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}</p>
            </div>
          </div>
          ...
          <div className="p-2.5 sm:p-3.5 bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] sm:text-xs font-black text-slate-900 dark:text-white mt-0.5 truncate">
                {serviceAlerts.isStandardMode
        ...
        <div className="bg-slate-50 dark:bg-slate-950/20 shrink-0 border-b border-slate-200 dark:border-white/5 px-4 sm:px-6 md:px-8 flex items-center justify-between py-2 sm:py-0">
          ...
          <div className="block sm:hidden w-full relative">
            ...
            <select
              ...
              className="w-full bg-white dark:bg-[#0d1324] border border-slate-300 dark:border-white/10 text-slate-700 dark:text-slate-200 px-3.5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider focus:outline-none focus:border-brand-primary/50 appearance-none cursor-pointer"
            >
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeTab === tab.id 
                      ...
                      : "bg-transparent text-slate-400 border-transparent hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900"
                  )}
                >
            ...
            >
              ...
              {activeTab === 'overview' && !isEditing && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 relative overflow-hidden group">
                      <div className="absolute top-0 right-0 p-8 opacity-5 text-slate-900 dark:text-white pointer-events-none group-hover:scale-110 duration-500 ease">
                        <Car size={96} />
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-2">
                        <div className="p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl">
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl col-span-1 sm:col-span-2 relative">
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Global Chassis VIN Profile</p>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                            <div className="bg-white dark:bg-slate-900/55 p-2.5 rounded-xl border border-slate-200 dark:border-white/5">
                              ...
                              <div className="flex items-center justify-between mt-1 gap-2">
                                ...
                                {formData.vin && (
                                  <button 
                                    ...
                                    className="p-1 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white rounded transition-colors"
                                    title="Copy Full VIN"
                                  >
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl">
                          ...
                          <p className="text-base sm:text-lg font-black text-slate-900 dark:text-white mt-1 flex items-center gap-2">
                    ...
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6">
                      ...
                      {oilAnalysis ? (
                        oilAnalysis.hasData ? (
                          <div className="space-y-4 sm:space-y-6">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                              <div className="p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl flex items-center justify-between">
                                <div>
                                ...
                                <span className="text-[10px] font-mono font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-900 px-2 py-1 rounded-md border border-slate-200 dark:border-white/5 shrink-0">
                            ...
                            <div className="p-4 sm:p-5 bg-gradient-to-br from-indigo-50 to-slate-50 dark:from-indigo-950/40 dark:to-slate-900/40 border border-brand-primary/20 rounded-xl sm:rounded-2xl relative overflow-hidden">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div className="space-y-1">
                                  ...
                                  <span className="font-black text-base sm:text-lg text-slate-900 dark:text-white">{oilAnalysis.nextDueDateLabel ?? 'N/A'}</span>
                                </div>
                                <div className="space-y-1">
                                  <span className="text-[10px] text-slate-500 uppercase font-bold block">Estimated Due Mileage</span>
                                  <span className="font-black text-base sm:text-lg text-slate-900 dark:text-white">{oilAnalysis.nextMileage?.toLocaleString()} mi</span>
                                </div>
                              </div>
                            </div>
                          </div>
                        ) : (
                          <div className="p-6 bg-slate-50 dark:bg-slate-950/20 border border-dashed border-slate-200 dark:border-white/5 rounded-2xl text-center space-y-2">
                            <Info size={24} className="text-slate-500 mx-auto" />
                            <p className="text-sm font-black text-slate-600 dark:text-slate-300">Statistical Engine Pending</p>
                        ...
                        <div className="p-6 bg-slate-50 dark:bg-slate-950/20 border border-dashed border-slate-200 dark:border-white/5 rounded-2xl text-center space-y-2">
                          ...
                          <p className="text-sm font-black text-slate-600 dark:text-slate-300">History Database Empty</p>
                          <p className="text-xs text-slate-500 leading-relaxed max-w-sm mx-auto">
                    ...
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 mt-4 sm:mt-6">
                      ...
                      <div className="space-y-3">
                        <textarea
                          ...
                          className="w-full bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800/80 rounded-xl p-3.5 text-xs font-semibold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-brand-primary h-28 resize-none placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:border-transparent transition-all"
                  ...
                  <div className="space-y-4 sm:space-y-6 lg:space-y-8">
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-6">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-3">
                        <UserIcon size={14} className="text-brand-secondary" /> Primary Contacts
                      </p>

                      <div className="space-y-3.5 sm:space-y-4">
                        {/* Mobile Phone Card */}
                        <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center justify-between group hover:border-brand-primary/20 transition-all">
                          <div className="space-y-0.5 truncate pr-2">
                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Mobile Phone</p>
                            <p className="text-sm font-extrabold text-slate-900 dark:text-white truncate font-mono">{formData.phone || 'Unknown'}</p>
                          </div>
                          {formData.phone && (
                            <a 
                              href={`tel:${formData.phone}`} 
                              className="w-8 h-8 rounded-lg bg-brand-primary/10 hover:bg-brand-primary text-brand-secondary hover:text-white flex items-center justify-center transition-all shrink-0 border border-slate-200 dark:border-white/5"
                            >
                              <Phone size={14} />
                            </a>
                          )}
                        ...
                        <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center justify-between group hover:border-brand-primary/20 transition-all">
                          <div className="space-y-0.5 truncate pr-2">
                            ...
                            <p className="text-sm font-extrabold text-slate-900 dark:text-white truncate font-mono">{formData.email || 'Not Provided'}</p>
                          </div>
                          {formData.email && (
                            <a 
                        ...
                        <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex items-center justify-between group hover:border-brand-primary/20 transition-all">
                          ...
                          {formData.address && (
                            <a 
                              ...
                              target="_blank" 
                              rel="noreferrer" 
                              className="w-8 h-8 rounded-lg bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white flex items-center justify-center transition-all shrink-0 border border-slate-200 dark:border-white/5"
                            >
                              <MapPin size={14} />
                            </a>
                          )}
                    ...
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-3">
                      ...
                      <div className="space-y-3.5 text-xs">
                        <div className="flex justify-between items-center text-slate-400">
                          ...
                          <span className="font-black text-slate-900 dark:text-white">{customer.soldByUsername || 'Import Conduit'}</span>
                        </div>
                        <div className="flex justify-between items-center text-slate-400">
                          <span className="font-bold">Dealership Source:</span>
                          <span className="font-black text-slate-900 dark:text-white font-mono">HY-{customer.dealershipId ? customer.dealershipId.slice(-6).toUpperCase() : 'MAIN'}</span>
                        </div>
                        <div className="flex justify-between items-center text-slate-400">
              ...
              {activeTab === 'demographics' && !isEditing && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                  ...
                  <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-6">
                    <h4 className="text-xs font-black text-slate-600 dark:text-slate-300 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-3">
                      <UserIcon size={14} className="text-brand-primary" /> Profile Identification
                    </h4>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl text-slate-700 dark:text-slate-200">
                      ...
                      <div className="col-span-1 sm:col-span-2 p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl text-slate-700 dark:text-slate-200">
                  ...
                  <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl">
                        <span className="text-[9px] font-black uppercase tracking-wider text-slate-500">Street Address</span>
                        <p className="text-sm font-extrabold text-slate-900 dark:text-white mt-1">{formData.address || 'Not Logged'}</p>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="p-3 sm:p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl text-slate-700 dark:text-slate-200">
                      ...
                      <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl text-xs text-slate-400 flex items-start gap-3">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4 sm:space-y-6">
                      ...
                      <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="space-y-1 pr-4">
                          <p className="text-xs font-extrabold text-slate-900 dark:text-white">Service Alert Triggers</p>
                              ...
                              <div className="space-y-4">
                                <div className="space-y-1.5">
                                  ...
                                  <select
                                    ...
                                    className="w-full bg-white dark:bg-[#0e1424] border border-rose-500/20 text-slate-700 dark:text-slate-200 px-3 py-2.5 rounded-lg text-xs font-bold focus:outline-none focus:border-rose-500/50"
                                  >
                                ...
                                <div className="space-y-1.5">
                                  ...
                                  <textarea
                                    ...
                                    rows={3}
                                    className="w-full bg-white dark:bg-[#0e1424] border border-rose-500/20 text-slate-700 dark:text-slate-200 px-3 py-2.5 rounded-lg text-xs placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:border-rose-500/50"
                                  />
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3 pt-1">
                                  <button
                                    ...
                                    className="py-2.5 text-xs font-black uppercase text-slate-600 dark:text-slate-400 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-750 border border-slate-300 dark:border-white/5 rounded-lg transition-all"
                                  >
                                    Cancel
                                  </button>
                                  <button
                    ...
                    {formData.stopAlertInfo && (
                      <div className="bg-rose-500/5 border border-rose-500/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4">
                        ...
                        <div className="space-y-3 text-xs text-rose-300/80">
                          <p className="leading-relaxed">
                            <span className="font-extrabold text-slate-900 dark:text-white block">Suspended Code / Reason</span>
                            {formData.stopAlertInfo.reason}
                          </p>
                          {formData.stopAlertInfo.notes && (
                            <p className="leading-relaxed">
                              <span className="font-extrabold text-slate-900 dark:text-white block">Notes / Observations</span>
                              {formData.stopAlertInfo.notes}
                            </p>
                          )}
                  ...
                  <div className="space-y-4 sm:space-y-6 lg:space-y-8">
                    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="p-3.5 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl">
                        ...
                        <div className="p-3.5 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl">
                          ...
                          <span className="text-xs font-black text-slate-900 dark:text-white mt-1 block">
                        ...
                        <div className="p-3.5 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl">
                          ...
                          <span className="text-xs font-black text-slate-600 dark:text-slate-300 mt-1 block">
              ...
              {isEditing && (
                <div className="bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-3xl p-4 sm:p-6 space-y-6 sm:space-y-8 max-w-4xl mx-auto">
                  <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/5 pb-4">
                    <div>
                      <h4 className="text-xs font-black text-slate-600 dark:text-slate-300 uppercase tracking-[0.2em] flex items-center gap-2">
                  ...
                  <div className="space-y-6 sm:space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          <label className="input-label font-bold text-slate-600 dark:text-slate-300">Delivery Date</label>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center justify-between">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <textarea
                          ...
                          className="w-full bg-white dark:bg-[#0e1324] border border-slate-300 dark:border-white/5 focus:border-brand-primary/50 text-slate-700 dark:text-slate-200 p-4 rounded-xl text-xs font-semibold focus:outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600 transition-all h-28 resize-none"
