    ...
    <div ref={rootRef} className="relative w-full max-w-md">
        ...
        <div className="relative">
          ...
          {query ? (
            <button
              type="button"
              onClick={handleClear}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 rounded-lg text-slate-500 hover:text-white hover:bg-slate-800 transition-colors"
      ...
      {open && query.trim() ? (
        <div
          id={listboxId}
          role="listbox"
          className="absolute z-50 mt-2 w-full rounded-xl border border-slate-800 bg-slate-950 shadow-2xl shadow-black/40 overflow-hidden"
        >
          {matches.length === 0 ? (
