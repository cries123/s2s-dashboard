...
export function ForecastMiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-800/80 bg-slate-950/50 px-4 py-3">
      ...
      <p className="text-sm font-black text-white tabular-nums mt-1">{value}</p>
    </div>
  );
}
...
export const forecastReadonlyClass =
  'w-full bg-slate-900/80 border border-slate-800 rounded-lg px-3 py-2 text-sm text-brand-primary text-center font-black tabular-nums';
